...
for i in range(2):
    if isItTable_h5p_content == two_tables[i]:
    ...
    query += f"END\n" + (",version = 'new'\n" if isItTable_h5p_content != two_tables[i] else "")

    query += f"WHERE library_id IN ({old_ids})\n"

    query += "AND " + ("id" if isItTable_h5p_content == two_tables[i] else f"content_id") + f" <= {last_id};\n"
    ...
    output_file = f"{file_path}{'_h5p_contents' if isItTable_h5p_content == two_tables[i] else '_h5p_contents_libraries'}.txt"
